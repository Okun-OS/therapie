...
/**
 * §145 Der Hilfe-Assistent an einer Stelle.
 *
 * Er wird von zwei Seiten gerufen: aus dem Fenster hinter dem Fragezeichen
 * (§140) und aus dem Gespräch „OKUN Assistent" unter Nachrichten (§145). Zwei
 * Kopien desselben Systemtexts wären schon nach dem ersten Umbau
 * auseinandergelaufen — und dann bekäme derselbe Mensch auf dieselbe Frage
 * zwei verschiedene Antworten, je nachdem, wo er sie stellt.
 *
 * Was hier NICHT passiert: Der Assistent liest keine Betriebsdaten. Er erklärt
 * das Programm. Wer Dienstpläne ändern lassen will, nutzt den OKUN Assistenten
 * mit Datenzugriff — das ist eine andere Tür mit anderen Rechten.
 */
...
export function assistentEingerichtet(): boolean {
 ...
 * Eine Antwort holen.
 *
 * Wirft nicht: Der Aufrufer bekommt entweder eine Antwort oder `null`. Beide
 * Aufrufer stehen in einem Gespräch, und ein Gespräch, in dem auf eine Frage
 * gar nichts folgt, ist schlimmer als eines mit einer ehrlichen Absage.
 */
export async function frageAssistenten(
  verlauf: AssistentNachricht[],
  seite?: string,
): Promise<string | null> {
  if (!assistentEingerichtet() || verlauf.length === 0) return null

  const system = seite
    ? `${SYSTEM_PROMPT}\n\n## Aktuelle Seite des Nutzers\nDer Nutzer befindet sich gerade auf: ${seite}`
    : SYSTEM_PROMPT
