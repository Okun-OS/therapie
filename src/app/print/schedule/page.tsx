...
function calcHours(startTime: string, endTime: string, breakThreshold: number, breakDeduction: number): number {
  const [sh, sm] = startTime.split(':').map(Number)
  const [eh, em] = endTime.split(':').map(Number)
  let mins = (eh * 60 + em) - (sh * 60 + sm)
  if (mins <= 0) mins += 1440
  if (mins >= breakThreshold) mins -= breakDeduction
...
export default function SchedulePrintPage() {
  ...
  if (error) return (
    <div className="p-8 text-center text-gray-500">
      Keine Druckdaten gefunden. Bitte kehre zur Dienstplan-Seite zur&uuml;ck und klicke erneut auf &quot;PDF&quot;.
    </div>
  )

  if (!data) return (
    <div className="p-8 text-center text-gray-400">Lade Druckdaten&hellip;</div>
  )

  const breakThreshold = data.breakThresholdMinutes ?? 360
  const breakDeduction = data.breakDeductionMinutes ?? 30
  ...
  return (
    <>
      <style>{`
        ...
        body { font-family: system-ui, sans-serif; background: white; color: #1a1a2e; margin: 0; }
      `}</style>
      ...
      <div className="no-print flex items-center gap-3 px-6 py-4 border-b border-gray-200 bg-white sticky top-0 z-10">
        ...
        <span className="text-xs text-gray-400 ml-2">Tipp: Im Druckdialog &bdquo;Als PDF speichern&ldquo; w&auml;hlen</span>
      </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-end justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Dienstplan &middot; {data.periodLabel}</p>
          </div>
            ...
            <div key={weekIdx} className={weekIdx > 0 ? 'mt-8 page-break-before' : ''}>
              {data.weeks.length > 1 && (
                <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">
                  Woche {weekIdx + 1} &middot; {formatDay(week.dates[0])} &ndash; {formatDay(week.dates[week.dates.length - 1])}
                </p>
              )}
                ...
                <tbody>
                  {data.employees.map((emp, empIdx) => {
                    ...
                    const weekHours = week.dates.reduce((sum, d) => {
                      const a = empAssign[d]
                      return a ? sum + calcHours(a.startTime, a.endTime, breakThreshold, breakDeduction) : sum
                    }, 0)
                    return (
                      <tr
                        key={emp.id}
                            ...
                            <td key={d} className="px-1 py-1.5 text-center">
                              {a ? (
                                <div className="rounded-md px-1 py-1" style={{ backgroundColor: '#eef2ff' }}>
                                  <div className="text-[10px] font-bold" style={{ color: '#1a1a2e' }}>{a.startTime}&ndash;{a.endTime}</div>
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-1 px-1">
                        {data.shifts.map(s => (
                          <span key={s.id} className="text-[9px] text-gray-500">
                            <span className="font-semibold text-[#1a1a2e]">{s.name}</span> {s.startTime}&ndash;{s.endTime}
                          </span>
                        ))}
                      </div>
                    </td>
        ...
        <div className="mt-8 border-t border-gray-200 pt-4">
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2">Stunden&uuml;bersicht Gesamt</p>
