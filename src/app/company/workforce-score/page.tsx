...
export default function CompanyWorkforceScore() {
  ...
  const load = async () => {
    ...
    try {
      const [lbRes, bonusRes] = await Promise.all([
        fetch('/api/workforce-score/leaderboard?scope=organization').then(r => r.json()),
  ...
  return (
    <>
      <Header title="Workforce Score" subtitle="Ranking über alle Standorte" />
      <div className="p-4 sm:p-6 space-y-5">
        <Card padding="sm">
          <div className="flex items-center gap-2 mb-3">
            <Trophy size={16} className="text-brand-dark" />
            <p className="text-sm font-semibold text-navy">Rangliste (Organisation)</p>
          </div>
          {loading ? (
